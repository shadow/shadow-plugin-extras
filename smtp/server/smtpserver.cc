...
#define loginst(level, inst, fmt, ...)                                  \
    do {                                                                \
        logfn(SHADOW_LOG_LEVEL_##level, __func__, "(ln %d, smtpserver= %d): " fmt,  \
...
uint32_t smtpserver_t::nextInstNum = 0;
...
{
    int rv;
    smtpserver_t *s = (smtpserver_t *)(ptr);
...
void
smtpserver_t::start(int argc, char *argv[])
{
    int opt;
    int long_index;
    uint16_t listenport = 25;
    ...
    logfn(SHADOW_LOG_LEVEL_MESSAGE, __func__,
          "smtpserver listening on port %u", listenport);
...
void
smtpserver_t::activate(const bool blocking)
{
    if (blocking) {
        evbase_->dispatch();
    } else {
